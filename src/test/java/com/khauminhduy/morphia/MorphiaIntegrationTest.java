package com.khauminhduy.morphia;
...
import com.khauminhduy.morphia.domain.Author;
import com.khauminhduy.morphia.domain.Book;
import com.khauminhduy.morphia.domain.Publisher;
...
public class MorphiaIntegrationTest {
	...
	public static void setup() {
		...
		morphia.mapPackage("com.khauminhduy.morphia");
	...
	public void givenDocument_whenUpdated_thenUpdateReflected() {
		...
		List<Book> books = datastore.createQuery(Book.class).field("title").contains("Learning Java").find().toList();
		assertEquals(4.95, books.get(0).getCost(), 2);
	}
	...
	public void givenDocument_whenAggregated_thenResultsCollected() {
		...
		Iterator<Author> authors = datastore.createAggregation(Book.class)
			.group("author", Group.grouping("book", Group.push("title")))
			.out(Author.class);
	...
	public void givenDocument_whenProjected_thenOnlyProjectionReceived() {
		...
		List<Book> books = datastore.createQuery(Book.class).field("title").contains("Learning Java").project("title", true).find().toList();
